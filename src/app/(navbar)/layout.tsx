  ...
  return (
    <header>
      <nav className="border border-b">
        <div className="max-w-7xl mx-auto px-6 md:px-16 lg:px-36 text-secondary">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="hidden md:block">
              <div className="ml-10 flex items-baseline space-x-4">
                {navigations.map((nav) => (
                  <Link key={nav.id} href={nav.href} className="hover:text-primary px-3 py-2 text-sm font-medium">
                    <span className={currentPath === nav.href ? 'text-primary' : 'text-secondary'}>{nav.title}</span>
                  </Link>
                ))}
              </div>
            </div>
